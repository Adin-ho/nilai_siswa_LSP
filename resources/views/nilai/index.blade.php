...
<div class="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
    <div>
        <h1 class="text-3xl font-black">
            {{ $role === 'siswa' ? 'Nilai Pribadi' : 'Data Nilai' }}
        </h1>

        <p class="mt-1 text-slate-500">
            @if ($role === 'admin')
                Admin dapat mengelola semua data nilai siswa.
            @elseif ($role === 'guru')
                Guru hanya dapat mengelola nilai pada mata pelajaran yang diampu.
            @else
    ...
    @if ($canManageNilai)
        <a href="{{ route('nilai.create') }}"
            class="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 font-bold text-white shadow-lg shadow-slate-200 transition hover:-translate-y-0.5 hover:bg-blue-700">
            + Tambah Nilai
        </a>
    @endif
</div>
...
<div class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
    <div class="border-b p-5">
        <form method="GET" class="flex gap-2">
            ...
            <button class="rounded-2xl bg-slate-900 px-5 py-3 font-bold text-white">
                Cari
            </button>
        </form>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('nilai.destroy', $item) }}" method="POST"
                                        ...
                                        <button class="rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-red-700 hover:bg-red-100">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                ...
                @empty
                    <tr>
                        <td colspan="{{ $canManageNilai ? 13 : 12 }}"
