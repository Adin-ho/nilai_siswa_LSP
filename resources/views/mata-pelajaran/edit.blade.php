...
@section('content')
<h1 class="mb-6 text-2xl font-black text-slate-900">Edit Mata Pelajaran</h1>
...
<div class="max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
    <form action="{{ route('mata-pelajaran.update', $mataPelajaran) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="kode_mapel" value="{{ old('kode_mapel', $mataPelajaran->kode_mapel) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100" required>
        </div>

        <div>
            <label class="mb-2 block text-sm font-black text-slate-700">Nama Mata Pelajaran</label>
            <input type="text" name="nama_mapel" value="{{ old('nama_mapel', $mataPelajaran->nama_mapel) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100" required>
        </div>
        ...
        <div>
            ...
            <select name="guru_id" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                ...
                @foreach ($gurus as $guru)
                    <option value="{{ $guru->id }}" @selected(old('guru_id', $mataPelajaran->guru_id) == $guru->id)>{{ $guru->nama_guru }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="keterangan" rows="3" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">{{ old('keterangan', $mataPelajaran->keterangan) }}</textarea>
        </div>
